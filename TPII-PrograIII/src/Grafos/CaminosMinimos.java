...
public class CaminosMinimos {
	GrafoDirTDA <Integer> Dijstra(GrafoDirTDA <Integer> g,int nodo)
	{
		...
		gr.InicializarGrafo();
		cp.sacar(nodo);		
		//agrego todos los vertices a gr y si existe las aristas nodo y sus adyacentes... 
			...
			while(!cp.conjuntoVacio())
			{
				...
				if(mejorP!=1)//si es igual a -1 es xq no existe la arista... 
				{
					...
					while(!c_aux.conjuntoVacio())
					{
						auxC=c_aux.elegir();
						c_aux.sacar(auxC);
						cp.agregar(auxC);//para verificar con el nuevo vertice qeu puse, si hay mejores caminos...
						...
						if(g.ExisteArista(mejorV, auxC))
						{
							if(!gr.ExisteArista(nodo, auxC))
							{
								...
								if(gr.PesoArista(nodo, mejorV)+g.PesoArista(mejorV,auxC)<gr.PesoArista(nodo, auxC))
								{
									gr.AgregarArista(nodo,auxC, gr.PesoArista(mejorV, auxC)+g.PesoArista(nodo, auxC));
								}
							}
